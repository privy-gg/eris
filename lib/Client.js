    ...
    */
    createMessage(channelID, content, file) {
        ...
        if (channel && channel.guild) {
            if (this.user) {
                if (!channel.permissionsOf(this.user.id).has('readMessages') 
                    || !channel.permissionsOf(this.user.id).has('viewChannel')
                    || !channel.permissionsOf(this.user.id).has('sendMessages')
                  ) return Promise.reject(new Error('Invalid permissions (createMessage check)'))
            }
        }
